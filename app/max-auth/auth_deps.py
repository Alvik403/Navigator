...
# Keycloak username → app.users.id (demo mapping; extend via env or DB later)
KC_USERNAME_TO_APP_USER: dict[str, str] = {
    ...
    "teacher.demo": "33333333-3333-3333-3333-333333333333",
}

HR_ROLES = {"hr_manager"}
TEACHER_ROLES = {"teacher"}
APP_HR_ROLES = {"hr", "admin"}
APP_TEACHER_ROLES = {"teacher", "admin"}
...
async def require_teacher(user: AuthUser = Depends(get_auth_user)) -> AuthUser:
    if user.app_role_code not in APP_TEACHER_ROLES and not TEACHER_ROLES.intersection(user.roles):
